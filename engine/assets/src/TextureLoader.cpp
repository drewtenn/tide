...
validate_header(std::span<const std::byte> bytes) noexcept {
    ...
    if (header.kind != static_cast<std::uint32_t>(AssetKind::Texture)) {
        return tide::unexpected{AssetError::KindMismatch};
    }
    if (header.schema_version != kTextureSchemaVersion) {
    ...
    if (header.payload_size < sizeof(TexturePayload)) {
        return tide::unexpected{AssetError::InvalidFormat};
    }
...
TextureLoader::load(Uuid /*uuid*/, std::span<const std::byte> cooked_bytes) {
...
void TextureLoader::unload(OpaquePayload /*payload*/) noexcept {
    // Same lifetime model as MeshLoader: the mmap owns the bytes.
}
